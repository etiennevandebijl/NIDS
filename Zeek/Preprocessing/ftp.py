...
"""
This module is used to proprocess ftp.log
"""
...
def preprocessing_ftp(ftp_log):
    ...
    ftp_log = common_used_practice(ftp_log, "reply_code_d1", COMMON_REPLY_CODE_D1)
    ftp_log = common_used_practice(ftp_log, "reply_code_d2", COMMON_REPLY_CODE_D2)
...
def _combine_ftp(ftp_log):
    ...
    """
    uid_interval = ftp_log.groupby("uid")["ts"].agg([np.min, np.max]).reset_index()
...
#ftp_log = merge_bro_log_files(get_data_folder("ISCX-IDS-2012", "BRO", "1_Raw"), "ftp")
#ftp_log_new = preprocessing_ftp(ftp_log)
