...
"""
This module is used to transform anomaly scores to a dynamic graph dataset.
"""
import glob
...
def time_iterator(dataset):
    ...
    for i in tqdm(range(len(time_epochs)-1)):
        ...
        if "ts_" in dataset.columns:
            dataset_t_i = dataset[((dataset["ts_"] > t_i) & (dataset["ts"] <= t_j))]
        else:
            dataset_t_i = dataset[((dataset["ts"] > t_i) & (dataset["ts"] <= t_j))]
        ...
        if dataset_t_i.shape[0] > 0:
            ...
            for edge, group in grouped_by_edge:
                ...
                info = list(edge) + [t_j, group.shape[0], lbl] + group[MODELS].max().values.tolist()
                results.append(info)
