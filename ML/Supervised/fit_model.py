...
from ML.Supervised.eval import feature_importance_statistics, evaluation_metrics
...
def perform_train_validation(df, models, splitter):
    ...
    for model, settings in models.items():
        ...
        y_pred, fitted_models, predicted_indices = apply_splitter(X, y, clf, splitter)

        df_scores = evaluation_metrics(y[predicted_indices], y_pred[predicted_indices], labels)
        df_fi = feature_importance_statistics(fitted_models, model, feature_names)
