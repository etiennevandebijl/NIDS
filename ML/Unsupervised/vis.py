...
def boxplot_scores_label(df, model, path = None):
    ...
    sns.boxplot(x="Label", y=model, data=df, ax=ax, palette="Set2", fliersize=0)
...
def interpret_scores(df_scores, n, P, model = "", output_path = None):
    ...
    cols = df_scores.columns.tolist()  + ["Baseline: "      + val[0],
                                          "Anomaly Ratio: " + val[1] + " F1 Score: " +  val[2], 
                                          "Max F1 Score: "  + val[3], 
                                          "Argmax Recall: " + val[4]]
    plt.legend(cols)
...
def scatter_plot_ts_scores(df, model, path = None, date = None):
    ...
    sns.scatterplot(y=model, x="ts", hue ="Label" ,data=df[df["Label"]=="Benign"], ax=ax, palette = color_dict, s = 15)
    sns.scatterplot(y=model, x="ts", hue ="Label" ,data=df[df["Label"]!="Benign"], ax=ax, palette = color_dict, s = 40)
    ...
    texts = ["MEDIAN MALICIOUS", "MEDIAN BENIGN", "95th PERCENTILE BENIGN", "5th PERCENTILE MALICIOUS"]
    ...
    for i in range(len(texts)):
        if (y_values[i] < y_max and y_values[i] > y_min):
            ...
            ax.text(x_pos[i], pos-0.01, texts[i], transform=ax.transAxes, fontsize=10, bbox = props)
    ...
    plt.xlabel("Time")
    plt.ylabel("Anomaly Score")
    plt.title("Anomaly Scores over Time per Label " + str(date.day) + " " + str(date.strftime("%B") + " "+ str(date.year)))
...
def score_per_class(pvt, class_dist, n, path = None, threshold = ""):
    g = pvt.plot(kind='bar', stacked=True, figsize=(min(pvt.shape[0]*3,15),9), width=0.85)
    ...
    for index, row in pvt.iterrows():
        ...
        if perc_b > 5: 
            g.text(i,  perc_b / 2, str(round(perc_b,1))+'%', color='white', ha="center")
        if perc_m > 5:
            g.text(i,  perc_b + perc_m / 2, str(round(perc_m,1))+'%' , color='white', ha="center")
        perc_total = int(class_dist.to_dict()[index] * n / 100)
        g.text(i, 103.5, "n: "+'{:,}'.format(perc_total) , color='black', ha="center")
        g.text(i, 101, str(round(perc_total * 100 /n,2))+"%", color='black', ha="center")
        i = i + 1
    ...
    plt.title("Prediction Accuracy per Label using Threshold Level "+threshold, fontsize = 10)
...
#input_path  = get_data_folder("UNSW-NB15", "BRO", "3_Connection") + "ftp_usl.csv"
...
#df["Predicted"] = np.where(df["Prediction"] == 1, "Predicted Malicious","Predicted Benign") 
#pvt = pd.pivot_table(df, index = "Label", columns = "Predicted", values = "Prediction", aggfunc = "count").fillna(0)
